      ...
      <div className="w-full">
        <div className="flex flex-col items-start relative">
          ...
          <div 
            ...
            style={{ 
              height: `${Math.max(((currentStep - 1) / (steps.length - 1)) * 100, 0)}%` 
            }} 
          />
          
          {steps.map((step, index) => (
            <div key={step.id} className={`flex items-start mb-8 relative w-full ${index === steps.length - 1 ? '' : 'pb-6'}`}>
              <button
                ...
                className={cn(
                  "w-10 h-10 rounded-full flex items-center justify-center text-sm font-medium transition-all duration-200 z-10",
                  {
                    ...
                    "bg-white border-2 border-gray-300 text-gray-700 hover:border-blue-500 hover:text-blue-500":
                      currentStep !== step.id && !completedSteps.includes(step.id) && canGoToStep(step.id),
                    "bg-gray-100 border-2 border-gray-200 text-gray-400 cursor-not-allowed": !canGoToStep(step.id),
                  },
                )}
              >
              ...
              <div className="ml-4">
                <p
                  className={cn("font-medium", {
                    ...
                    "text-gray-900": currentStep !== step.id && !completedSteps.includes(step.id),
                  })}
                >
                  {step.title}
                </p>
    ...
    <div className="py-6 w-full">
      <div className="hidden md:flex items-center justify-between relative w-full">
        ...
        <div 
          ...
          style={{ 
            width: `${Math.max(((currentStep - 1) / (steps.length - 1)) * 100, 0)}%` 
          }} 
        />
        
        {steps.map((step, index) => (
          <div key={step.id} className="relative z-10 flex flex-col items-center w-full max-w-[150px]">
            <button
              onClick={() => canGoToStep(step.id) && onStepClick(step.id)}
              disabled={!canGoToStep(step.id)}
              className={cn(
            ...
            <div className="mt-3 text-center w-full">
              <p
                className={cn("text-sm font-medium truncate", {
                  "text-blue-600": currentStep === step.id,
                  "text-green-600": completedSteps.includes(step.id),
                  "text-gray-700": currentStep !== step.id && !completedSteps.includes(step.id),
                })}
              >
                {step.title}
              </p>
      ...
      <div className="md:hidden">
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs font-medium text-gray-700">Step {currentStep} of {steps.length}</span>
              ...
              <div
                className={cn(
                  "w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium mb-2",
                  {
                    ...
                    "bg-green-500 text-white": completedSteps.includes(step.id),
                    "bg-white border-2 border-gray-300 text-gray-700": currentStep !== step.id && !completedSteps.includes(step.id),
